...
function tokensToTypes(tokens: Token[]): string[] {
...
describe("UnicodeTokenizer", () => {
  ...
  it("keeps script boundaries", () => {
    ...
    const t1 = tokenized.text.slice(
      tokenized.tokens[0].charInterval.startPos,
      tokenized.tokens[0].charInterval.endPos
    );
    const t2 = tokenized.text.slice(
      tokenized.tokens[1].charInterval.startPos,
      tokenized.tokens[1].charInterval.endPos
    );
...
describe("tokensText", () => {
  ...
  it("throws for invalid interval", () => {
    ...
    expect(() => tokensText(tokenized, { startIndex: -1, endIndex: 1 })).toThrow(
